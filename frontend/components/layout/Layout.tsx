...
const Layout: React.FC<LayoutProps> = ({ children }) => {
  ...
  const getLinkClass = (path: string) => {
    ...
    return `
      ...
      ${active 
        ? 'bg-amber-500 text-white shadow-lg shadow-amber-200 dark:shadow-amber-900/50 font-bold' 
  ...
  const NavLinks = () => (
    <>
      ...
      {user?.role === 'admin' && (
        <>
          <div className="text-xs font-bold text-slate-400 dark:text-slate-500 uppercase px-4 mt-8 mb-2 tracking-wider">Gerencia</div>
  ...
  return (
    <div className="flex h-screen bg-slate-50 dark:bg-slate-900 font-sans overflow-hidden">
      ...
      <aside className="hidden md:flex w-72 bg-white dark:bg-slate-800 border-r border-slate-200 dark:border-slate-700 flex-col p-6 fixed h-full z-20">
        <div className="flex items-center gap-3 px-2 mb-10">
          <div className="w-10 h-10 bg-amber-500 rounded-xl flex items-center justify-center text-white shadow-lg shadow-amber-200 dark:shadow-amber-900/50">
            <LayoutGridIcon size={24} />
          </div>
          <div>
            <h1 className="text-xl font-bold text-slate-800 dark:text-slate-100 tracking-tight">Primera Parada</h1>
            <p className="text-xs text-slate-400 dark:text-slate-500 font-medium uppercase tracking-wider">Restaurante</p>
          </div>
        </div>
      ...
      {isMobileMenuOpen && (
        <div className="fixed inset-0 z-50 md:hidden">
          ...
          {/* Panel deslizante */}
          <nav className="absolute left-0 top-0 bottom-0 w-3/4 max-w-xs bg-white dark:bg-slate-800 shadow-2xl p-6 flex flex-col animate-in slide-in-from-left duration-300">
            <div className="flex justify-between items-center mb-8">
              <div className="flex items-center gap-3">
                <div className="w-8 h-8 bg-amber-500 rounded-lg flex items-center justify-center text-white shadow-md">
                  <LayoutGridIcon size={18} />
                </div>
                <h1 className="text-lg font-bold text-slate-800 dark:text-slate-100">Menú</h1>
              </div>
              <button onClick={() => setIsMobileMenuOpen(false)} className="p-2 bg-slate-50 dark:bg-slate-700 rounded-full text-slate-500 dark:text-slate-400">
                <XIcon size={20} />
              </button>
            </div>
            ...
            <div className="mt-6 pt-6 border-t border-slate-100 dark:border-slate-700 space-y-3">
              <div className="flex items-center gap-3 mb-4 px-2">
                <div className="w-10 h-10 rounded-full bg-slate-100 dark:bg-slate-700 border border-slate-200 dark:border-slate-600 flex items-center justify-center text-slate-600 dark:text-slate-300 font-bold shrink-0">
      ...
      <main className="flex-1 md:ml-72 flex flex-col h-full w-full relative">
        ...
        {/* Header Móvil (Sticky) */}
        <div className="md:hidden bg-white/90 dark:bg-slate-800/90 backdrop-blur-md px-4 py-3 border-b border-slate-200 dark:border-slate-700 flex justify-between items-center sticky top-0 z-30 shadow-sm">
          <div className="flex items-center gap-3">
            {/* Botón Hamburguesa */}
            <button onClick={() => setIsMobileMenuOpen(true)} className="p-2 -ml-2 text-slate-600 dark:text-slate-300 hover:bg-slate-100 dark:hover:bg-slate-700 rounded-lg active:scale-95 transition-transform">
              <MenuIcon size={24} />
            </button>
            <h1 className="font-bold text-slate-800 dark:text-slate-100 text-lg">Primera Parada</h1>
          </div>
          <div className="w-8 h-8 bg-amber-100 dark:bg-amber-900 rounded-full flex items-center justify-center text-amber-700 dark:text-amber-300 font-bold text-sm border border-amber-200 dark:border-amber-700">
             {user?.name.charAt(0)}
          </div>
        </div>
        ...
        <nav className="md:hidden fixed bottom-0 left-0 right-0 bg-white dark:bg-slate-800 border-t border-slate-200 dark:border-slate-700 flex justify-around p-2 pb-safe z-40 shadow-[0_-4px_20px_rgba(0,0,0,0.1)]">
           <Link to={ROUTES.ORDERS} className={`flex flex-col items-center justify-center p-1 rounded-xl w-16 h-14 transition-colors ${location.pathname === ROUTES.ORDERS ? 'text-amber-600 bg-amber-50 dark:bg-amber-900/30' : 'text-slate-400 dark:text-slate-500'}`}>
           ...
           <Link to={ROUTES.KITCHEN} className={`flex flex-col items-center justify-center p-1 rounded-xl w-16 h-14 transition-colors ${location.pathname === ROUTES.KITCHEN ? 'text-amber-600 bg-amber-50 dark:bg-amber-900/30' : 'text-slate-400 dark:text-slate-500'}`}>
           ...
           <Link to={ROUTES.CASHIER} className={`flex flex-col items-center justify-center p-1 rounded-xl w-16 h-14 transition-colors ${location.pathname === ROUTES.CASHIER ? 'text-amber-600 bg-amber-50 dark:bg-amber-900/30' : 'text-slate-400 dark:text-slate-500'}`}>
           ...
           {user?.role === 'admin' ? (
             <Link to={ROUTES.DASHBOARD} className={`flex flex-col items-center justify-center p-1 rounded-xl w-16 h-14 transition-colors ${location.pathname === ROUTES.DASHBOARD ? 'text-amber-600 bg-amber-50 dark:bg-amber-900/30' : 'text-slate-400 dark:text-slate-500'}`}>
             ...
             <Link to={ROUTES.MENU} className={`flex flex-col items-center justify-center p-1 rounded-xl w-16 h-14 transition-colors ${location.pathname === ROUTES.MENU ? 'text-amber-600 bg-amber-50 dark:bg-amber-900/30' : 'text-slate-400 dark:text-slate-500'}`}>
